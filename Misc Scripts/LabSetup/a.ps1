using module '.\Modules\class_vdrive.psm1'
...
Class VSwitch
{
  ...
  VSwitch($vSwitch)
  {
    ...
    if($vSwitch.Type -eq 'External')
    {
      #Aditional Configuration Required

      ## TODO
      ##
      ## Select Server > Check for switch > not exists > request adapter selection
    }
    else
    {
...
Class Server
{
  ...
  Server($ServerTable)
  {
    ...
    Foreach($vSwitch in $ServerTable.Networking)
    {
      $this.Switches += [VSwitch]::New($vSwitch)
    }
